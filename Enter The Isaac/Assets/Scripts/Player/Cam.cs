...
public class Cam : MonoBehaviour {
    ...
    void ToCenterPos (float lerpSpeed) {
        ...
        if (crosshair.mouseControlled == true) {
            ...
            transform.rotation = Quaternion.Lerp (transform.rotation, Quaternion.Euler (startRot + (new Vector3 (-Input.GetAxis("AimVert") * rotAmount, Input.GetAxis("AimHor") * rotAmount, 0 * rotAmount) * PlayerPrefs.GetFloat ("mouseFollowAmount"))), Time.deltaTime * 5);
            centerPos += new Vector3 (Input.GetAxis("AimHor") * rotAmount / 15, 0, Input.GetAxis("AimVert") * rotAmount / 15) * followMouseAmount * PlayerPrefs.GetFloat ("mouseFollowAmount");
        }
